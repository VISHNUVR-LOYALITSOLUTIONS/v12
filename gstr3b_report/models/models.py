    ...
    def generate_3b_report(self, wb1):
        ...
        for invoice in self.sorted_invoices:
            for invoice_line in invoice.invoice_line_ids:
                ...
                if line_taxes:
                    line_amount = round(line_taxes['total_excluded'], 2) if invoice.type in (
                    'out_invoice', 'in_invoice') else (
                        round((line_taxes['total_excluded'] * -1), 2))
                # _logger.info(line_taxes)
                igst_amount = cgst_amount = sgst_amount = cess_amount = kfc_amount = gst_amount = 0.0
                ...
                line_total_amount = line_amount + igst_amount + cgst_amount + sgst_amount + cess_amount + kfc_amount

                if line_total_amount > 0 and invoice.type in ('in_refund', 'out_refund'):
                    line_total_amount = line_total_amount * -1

                if invoice.type in ('in_refund', 'out_refund'):
                    if line_total_amount < 0:
                        check_total = line_total_amount * -1
                    else:
                        check_total = line_total_amount
                    if line_amount < 0:
                        check_taxable = line_amount * -1
                    else:
                        check_taxable = line_amount
                ...
                if invoice.type in ('out_invoice', 'out_refund'):  # Customer Invoice
                    if float_compare(check_total, check_taxable,
                ...
                # TODO: Vendor Bills with reverse charge doesn't have tax lines filled, so it must be calculated
                    ...
                    if not invoice.partner_id.x_gstin:
                        ...
                        if float_compare(check_total, check_taxable, precision_digits=2) == 0:  # Zero GST taxes
        ...
        #         cgst_amount += gst_amount / 2
        #         sgst_amount += gst_amount / 2
        #         line_total_amount = pos_line.price_subtotal + igst_amount + cgst_amount + sgst_amount + cess_amount + kfc_amount
        #         if line_amount < 0:
        #             check_taxable = line_amount * -1
        #         else:
        #             check_taxable = line_amount
